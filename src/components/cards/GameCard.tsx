...
export function GameCard({
    ...
    return (
      ...
      >
        <div className={cardFrame}>
          <Link href={href} className="group -m-1 flex flex-col gap-2 rounded-xl p-1 active:scale-[0.99]">
            {body}
          </Link>
        </div>
        <Link
  ...
  return (
    <motion.div whileHover={{ y: -3 }} transition={{ type: "spring", stiffness: 420, damping: 30 }}>
      <Link href={href} className={cn("flex flex-col gap-2 active:scale-[0.99]", cardFrame, className)}>
        {body}
