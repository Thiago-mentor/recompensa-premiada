...
type ChestRarity = "comum" | "raro" | "epico" | "lendario";

export type WeightedPrizeConfig = {
  kind?: "coins" | "gems" | "rewardBalance" | "chest";
  coins: number;
  weight: number;
...
export function rouletteTableEntries(
  ...
  for (const row of rows) {
    ...
    if (segmentKind === "chest") {
      ...
      if (!r || !KNOWN_CHEST_ORDER.includes(r)) continue;
...
/** Resposta do servidor usada só para alinhar a roleta visual à fatia sorteada (cliente). */
...
function rowEffectiveKind(row: WeightedPrizeConfig): NonNullable<WeightedPrizeConfig["kind"]> | "coins" {
...
export function resolveMatchEconomy(
  ...
  if (gameId === "card_battle") {
    ...
    const resolved = applyRewardOverrides(
      resultado,
      win ? Math.min(90, Math.max(25, 34 + Math.floor(power / 2))) : 4,
      rankingPointsFrom(normalizedScore, resultado),
      rewardOverrides?.card_battle,
    );
    return {
      normalizedScore,
      rewardCoins: resolved.rewardCoins,
