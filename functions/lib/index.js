...
const VICTORY_RANKED_GAME_IDS = new Set(["ppt", "quiz", "reaction_tap", "card_battle"]);
...
const DEFAULT_PVP_CHOICE_SEC = { ppt: 10, quiz: 10, reaction_tap: 10, card_battle: 12 };
...
function parsePvpChoiceSecondsFromDoc(d) {
    ...
    return {
        ...
        card_battle: clampPvpChoiceSec(o.card_battle, DEFAULT_PVP_CHOICE_SEC.card_battle),
    };
}
...
const CARD_BATTLE_DECK = {
    guardiao: { id: "guardiao", name: "Guardiao", element: "terra", attack: 4, defense: 8 },
    lamina: { id: "lamina", name: "Lamina", element: "ar", attack: 8, defense: 3 },
    chama: { id: "chama", name: "Chama", element: "fogo", attack: 7, defense: 4 },
    mare: { id: "mare", name: "Mare", element: "agua", attack: 5, defense: 6 },
    raiz: { id: "raiz", name: "Raiz", element: "terra", attack: 6, defense: 5 },
    vento: { id: "vento", name: "Vento", element: "ar", attack: 5, defense: 7 },
    timeout: { id: "timeout", name: "Sem jogada", element: "neutro", attack: 0, defense: 0 },
};
function isCardBattleCardId(value) {
    return Object.prototype.hasOwnProperty.call(CARD_BATTLE_DECK, value) && value !== "timeout";
}
function cardBattleElementBonus(a, b) {
    if (a === "neutro" || b === "neutro" || a === b)
        return 0;
    if ((a === "agua" && b === "fogo") ||
        (a === "fogo" && b === "ar") ||
        (a === "ar" && b === "terra") ||
...
function resolveCardBattleRound(hostCardId, guestCardId) {
    const hostCard = CARD_BATTLE_DECK[hostCardId] ?? CARD_BATTLE_DECK.timeout;
    const guestCard = CARD_BATTLE_DECK[guestCardId] ?? CARD_BATTLE_DECK.timeout;
    const hostPower = hostCard.attack * 2 + hostCard.defense + cardBattleElementBonus(hostCard.element, guestCard.element);
    const guestPower = guestCard.attack * 2 + guestCard.defense + cardBattleElementBonus(guestCard.element, hostCard.element);
    const winner = hostPower > guestPower ? "host" : guestPower > hostPower ? "guest" : "draw";
    return { hostCard, guestCard, hostPower, guestPower, winner };
}
...
const RAFFLE_MAX_INSTANT_PRIZE_HITS = 1000;
...
function avatarRequirementProgress(data) {
    return {
...
function gameMatchCounterPatch(gameId) {
    if (gameId === "ppt")
        return { totalPptPartidas: firestore_2.FieldValue.increment(1) };
    if (gameId === "quiz")
        return { totalQuizPartidas: firestore_2.FieldValue.increment(1) };
    if (gameId === "reaction_tap")
        return { totalReactionPartidas: firestore_2.FieldValue.increment(1) };
    if (gameId === "card_battle")
        return { totalCardBattlePartidas: firestore_2.FieldValue.increment(1) };
    return {};
}
...
async function getEconomy() {
    ...
    const rawSaldoPointsPerReal = Math.floor(Number(d.saldoPointsPerReal ??
        d.cashPointsPerReal));
...
function resolveRewardedAdMonetaryParts(economy, placementId, kind) {
    const row = economy.rewardedAdRewardsByPlacement[placementId];
    const gems = row?.gems ?? 0;
    const rewardBalance = row?.rewardBalance ?? 0;
    if (kind === "generic") {
        const baseCoins = row && typeof row.coins === "number" && Number.isFinite(row.coins)
...
const RAFFLE_AD_COOLDOWN_MAX_SEC = 86400;
function normalizeRaffleRewardedAdCooldownSeconds(raw, entryMode) {
...
function normalizeRaffleInstantPrizeTiers(raw) {
    ...
    if (totalQuantity > RAFFLE_MAX_INSTANT_PRIZE_TOTAL_QUANTITY) {
        throw new https_1.HttpsError("invalid-argument", `A soma das quantidades premiadas não pode passar de ${RAFFLE_MAX_INSTANT_PRIZE_TOTAL_QUANTITY}.`);
    }
    return normalized;
}
function normalizeRaffleInstantPrizeHits(raw) {
    if (!Array.isArray(raw))
        return [];
    return raw
        .map((item) => {
        ...
        return {
            ...
            awardedAt: coerceTimestampOrNull(value.awardedAt) ?? firestore_2.Timestamp.fromMillis(0),
        };
    })
        .filter((item) => item != null);
}
function normalizeRaffleScheduleMode(raw, endsAt) {
    if (raw === "until_sold_out")
        return "until_sold_out";
    if (raw === "date_range")
        return "date_range";
...
function normalizeRaffleEntryMode(raw) {
...
function raffleDocFromFirestore(id, data) {
    ...
    return {
        ...
        rewardedAdCooldownSeconds: normalizeRaffleRewardedAdCooldownSeconds(data.rewardedAdCooldownSeconds, entryModeNorm),
...
function publicRaffleViewFromDoc(docSnap) {
...
function shuffleNumberArrayInPlace(nums) {
    for (let i = nums.length - 1; i > 0; i--) {
        const j = (0, node_crypto_1.randomInt)(0, i + 1);
        const tmp = nums[i];
        nums[i] = nums[j];
        nums[j] = tmp;
    }
...
function isRaffleSoldOut(raffle) {
    const allocatedCount = raffle.allocationMode === "random" ? raffle.soldCount : raffle.nextSequentialNumber;
    return allocatedCount >= raffle.releasedCount;
}
function shouldAutoCloseRaffle(raffle, nowMs) {
    if (raffle.status !== "active")
        return false;
    if (isRaffleSoldOut(raffle))
        return true;
...
function canUseSameDayFederalResult(parts) {
    if (parts.hour < 19)
        return true;
    if (parts.hour > 19)
        return false;
...
function scheduleFederalResultAt(closedAt, timeZone = DEFAULT_SCHEDULE_OPTS.timeZone) {
    ...
    for (let offset = 0; offset < 8; offset += 1) {
        ...
        if (!isFederalDay)
            continue;
        if (offset === 0 && !canUseSameDayFederalResult(closedParts))
            continue;
        return firestore_2.Timestamp.fromMillis(appDateTimeToUtcMs({
...
function resolveRaffleResultScheduledAt(raffle) {
    if (raffle.resultScheduledAt)
        return raffle.resultScheduledAt;
    if (!raffle.closedAt)
        return null;
    return scheduleFederalResultAt(raffle.closedAt, raffle.drawTimeZone);
}
function buildCloseRafflePayload(raffle, closedAt) {
    const payload = {
        status: "closed",
        closedAt,
...
function buildNoWinnerRafflePayload(winningNumber) {
    return {
        status: "no_winner",
        winningNumber,
        winnerUserId: null,
...
function normalizeWinningNumberForRaffle(raw, releasedCount) {
    const parsed = Math.floor(Number(raw));
    const max = Math.max(1, Math.min(RAFFLE_MAX_RELEASED_COUNT, Math.floor(releasedCount)));
    if (!Number.isFinite(parsed) || parsed < 0 || parsed >= max) {
        throw new https_1.HttpsError("invalid-argument", `Número vencedor inválido. Informe um número entre 0 e ${max - 1}.`);
    }
    return parsed;
}
function instantPrizeConfigSignature(tiers) {
...
function resolveInstantPrizeHitsForPurchase(input) {
    ...
    for (const ticketNumber of input.ticketNumbers) {
        ...
        const remainingPrizeSlots = tiers.reduce((sum, tier) => sum + Math.max(0, tier.quantity - tier.awardedCount), 0);
        if (remainingNumbers <= 0 || remainingPrizeSlots <= 0)
            break;
        const shouldAward = (0, node_crypto_1.randomInt)(0, remainingNumbers) < remainingPrizeSlots;
        if (shouldAward) {
            let prizeOffset = (0, node_crypto_1.randomInt)(0, remainingPrizeSlots);
            let tierIndex = 0;
...
function applyRewardedAdMonetaryGrantInTx(input) {
    ...
    const boosted = resolveBoostedCoins(baseCoins, input.u, input.economy);
    let coinsOut = 0;
    let boostOut = 0;
    if (boosted.totalCoins > 0) {
        coinsOut = boosted.totalCoins;
        boostOut = boosted.boostCoins;
        input.userPatch.coins = firestore_2.FieldValue.increment(boosted.totalCoins);
    ...
    if (gems > 0) {
        input.userPatch.gems = firestore_2.FieldValue.increment(gems);
    ...
    if (rewardBalance > 0) {
        input.userPatch.rewardBalance = firestore_2.FieldValue.increment(rewardBalance);
...
function clampChestAdSpeedupFixedMinutes(value, fallback) {
    const raw = Math.floor(Number(value));
    if (!Number.isFinite(raw))
        return fallback;
...
function chestAdSpeedupReducedMs(config, remainingMs) {
...
async function getChestSystemConfig() {
    ...
    return {
        ...
        adSpeedupFixedMinutes: clampChestAdSpeedupFixedMinutes(d.adSpeedupFixedMinutes, DEFAULT_CHEST_SYSTEM_CONFIG.adSpeedupFixedMinutes),
...
function normalizeWeightedPrizeTable(raw, fallback) {
    if (!Array.isArray(raw))
        return fallback;
    const chestRarities = ["comum", "raro", "epico", "lendario"];
    const rows = [];
    for (const item of raw) {
        ...
        if (kind === "chest") {
            ...
            if (!chestRarities.includes(r))
                continue;
            rows.push({ kind: "chest", coins: 0, weight, chestRarity: r });
            continue;
        }
...
function appDateTimeToUtcMs(parts, timeZone = DEFAULT_SCHEDULE_OPTS.timeZone) {
...
function normalizePublicNameModerationValue(value) {
    return value
        .normalize("NFD")
        .replace(/[\u0300-\u036f]/g, "")
        .toLowerCase()
...
function hasBlockedPublicNameTerm(value) {
...
function assertAllowedPublicName(value, fieldLabel) {
    if (hasBlockedPublicNameTerm(value)) {
        throw new https_1.HttpsError("invalid-argument", `${fieldLabel} não permitido. Evite palavrões, pornografia, ofensas ou termos inadequados.`);
    }
}
...
function isAllowedUserAvatarUrl(rawUrl, uid) {
    try {
...
function isOwnClanAssetUrl(rawUrl, uid) {
    try {
        const url = new URL(rawUrl);
        if (url.hostname !== "firebasestorage.googleapis.com")
            return false;
...
async function getReferralConfig() {
    ...
    const antiFraudSource = privateData.antiFraudRules && typeof privateData.antiFraudRules === "object"
        ? privateData
        : d;
    return {
        enabled: d.enabled !== false,
        ...
        antiFraudRules: {
            blockSelfReferral: antiFraudSource.antiFraudRules && typeof antiFraudSource.antiFraudRules === "object"
                ? antiFraudSource.antiFraudRules.blockSelfReferral !== false
                : true,
            flagBurstSignups: antiFraudSource.antiFraudRules && typeof antiFraudSource.antiFraudRules === "object"
                ? antiFraudSource.antiFraudRules.flagBurstSignups !== false
                : true,
            burstSignupThreshold: Math.max(1, Math.floor(Number(antiFraudSource.antiFraudRules && typeof antiFraudSource.antiFraudRules === "object"
...
exports.getReferralPublicConfig = (0, https_1.onCall)(DEFAULT_CALLABLE_OPTS, async (request) => {
...
function referralMeetsQualification(rules, userData, emailVerified) {
    ...
    if (Number(userData.serverVerifiedPvpMatches || 0) < rules.minMatchesPlayed)
        return false;
...
async function grantRewardedAdPlacement(input) {
    ...
    if (input.placementId === CHEST_SPEEDUP_PLACEMENT_ID) {
        throw new https_1.HttpsError("failed-precondition", "Este placement só pode ser usado pela função speedUpChestUnlock.");
    }
...
async function upsertRanking(input) {
    ...
    await db.runTransaction(async (tx) => {
        ...
        tx.set(userRef, {
            serverVerifiedPvpMatches: firestore_2.FieldValue.increment(1),
            serverVerifiedPvpWins: firestore_2.FieldValue.increment(input.win ? 1 : 0),
...
async function downloadImageForModeration(url) {
    const response = await fetch(url, {
        redirect: "manual",
        signal: AbortSignal.timeout(10000),
    });
    if (response.status >= 300 && response.status < 400) {
        throw new https_1.HttpsError("invalid-argument", "A imagem não pode redirecionar para outro endereço.");
    }
    if (!response.ok) {
        throw new https_1.HttpsError("invalid-argument", "Não foi possível baixar a imagem para moderação.");
    }
    const contentType = response.headers.get("content-type") || "";
    if (!contentType.startsWith("image/")) {
        throw new https_1.HttpsError("invalid-argument", "O arquivo enviado não parece ser uma imagem válida.");
    }
    const contentLength = Number(response.headers.get("content-length") || 0);
    if (contentLength > 4 * 1024 * 1024) {
        throw new https_1.HttpsError("invalid-argument", "A imagem precisa ter no máximo 4 MB para moderação.");
    }
...
async function assertAvatarImageAllowed(photoURL) {
    ...
    try {
        ...
        if (UNSAFE_SAFESEARCH_LEVELS.has(adult) ||
            UNSAFE_SAFESEARCH_LEVELS.has(racy) ||
            violence === "VERY_LIKELY") {
            throw new https_1.HttpsError("permission-denied", "Avatar recusado pela moderação automática. Envie uma foto sem nudez, conteúdo vulgar ou violência explícita.");
        }
    }
    catch (error) {
...
function readRaffleAdLastClaimMs(u, raffleId) {
    const map = u.raffleAdLastClaimAt;
    if (!map || typeof map !== "object" || Array.isArray(map))
        return 0;
    const v = map[raffleId];
    return millisFromFirestoreTime(v);
}
function assertCanClaimRaffleAdNumber(u, raffle, nowMs) {
    if (raffle.entryMode !== "rewarded_ad")
        return;
    ...
    if (elapsedSec < sec) {
        ...
        throw new https_1.HttpsError("resource-exhausted", `Aguarde ${wait}s para obter outro número por anúncio neste sorteio.`);
    }
}
 ...
 */
async function reservePvpEconomyEligibilityInTx(tx, input) {
    const day = dailyKey();
    const pair = [input.hostUid, input.guestUid].sort().join("|");
    const pairId = hashId("ranking_pair", day, input.gameId, pair);
    const matchId = hashId("ranking_match", input.gameId, input.roomId);
...
async function postCardBattleRankingFromWinner(roomId, hostUid, guestUid, matchWinner, hostPower, guestPower, forfeitMeta) {
    ...
    const baseMeta = {
        pvpRoomId: roomId,
        cardBattleMatchWinner: matchWinner,
        hostPower,
        guestPower,
    };
    if (forfeitMeta) {
        baseMeta.forfeit = true;
        baseMeta.forfeitedBy = forfeitMeta.forfeitedByUid;
    }
    const ecoH = (0, gameEconomy_1.resolveMatchEconomy)("card_battle", hostRes, hostPower, { ...baseMeta, cardPower: hostPower }, economyConfig.matchRewardOverrides);
    const ecoG = (0, gameEconomy_1.resolveMatchEconomy)("card_battle", guestRes, guestPower, { ...baseMeta, cardPower: guestPower }, economyConfig.matchRewardOverrides);
    const [hSnap, gSnap] = await Promise.all([
        db.doc(`${COL.users}/${hostUid}`).get(),
        db.doc(`${COL.users}/${guestUid}`).get(),
    ]);
...
async function applyCardBattleMatchCompletionInTransaction(tx, roomRef, roomId, r, matchWinner, hostCardId, guestCardId, hostPower, guestPower, forfeitByUid) {
    const hostUid = String(r.hostUid);
    const guestUid = String(r.guestUid);
    const hostScore = Number(r.cardBattleHostScore ?? 0);
    const guestScore = Number(r.cardBattleGuestScore ?? 0);
    const nextHostScore = Math.max(hostScore, Number(r.cardBattleNextHostScore ?? hostScore));
    const nextGuestScore = Math.max(guestScore, Number(r.cardBattleNextGuestScore ?? guestScore));
    const hostRes = matchWinner === "host" ? "vitoria" : "derrota";
    const guestRes = matchWinner === "guest" ? "vitoria" : "derrota";
    ...
    const metaBase = {
        pvpRoomId: roomId,
        cardBattleMatchWinner: matchWinner,
        cardBattleFinalHostScore: nextHostScore,
        cardBattleFinalGuestScore: nextGuestScore,
        hostCardId,
        guestCardId,
        hostPower,
        guestPower,
        ...(forfeitByUid ? { forfeit: true, forfeitedBy: forfeitByUid } : {}),
    };
    const economyConfig = await getEconomy();
    ...
    tx.update(hostUserRef, {
        ...
        ...gameMatchCounterPatch("card_battle"),
    ...
    if (boostedH.totalCoins > 0) {
        tx.set(wHost, {
            ...
            saldoApos: Number(hu.coins ?? 0) + boostedH.totalCoins,
            descricao: withBoostDescription("Batalha de cartas 1v1", boostedH.boostCoins),
    ...
    if (boostedG.totalCoins > 0) {
        tx.set(wGuest, {
            ...
            saldoApos: Number(gu.coins ?? 0) + boostedG.totalCoins,
            descricao: withBoostDescription("Batalha de cartas 1v1", boostedG.boostCoins),
    ...
    tx.update(roomRef, {
        status: "completed",
        phase: "completed",
        cardBattleHostScore: nextHostScore,
        cardBattleGuestScore: nextGuestScore,
        cardBattleLastHostCardId: hostCardId,
        cardBattleLastGuestCardId: guestCardId,
        cardBattleLastHostPower: hostPower,
        cardBattleLastGuestPower: guestPower,
        cardBattleLastRoundWinner: matchWinner,
        cardBattleMatchWinner: matchWinner,
        cardBattleOutcome: outcome,
        cardBattleRewardsApplied: true,
        cardBattlePickedUids: [],
        cardBattleEndedByForfeit: Boolean(forfeitByUid),
        ...(forfeitByUid ? { cardBattleForfeitedByUid: forfeitByUid } : {}),
    ...
    return { hostUid, guestUid, matchWinner, hostPower, guestPower };
}
async function applyCardBattleRoundResultInTransaction(tx, roomRef, roomId, r, hostCardId, guestCardId, cardWindowMs, pickRefs) {
    const resolved = resolveCardBattleRound(hostCardId, guestCardId);
    const hostScore = Number(r.cardBattleHostScore ?? 0);
    const guestScore = Number(r.cardBattleGuestScore ?? 0);
    const nextHostScore = hostScore + (resolved.winner === "host" ? 1 : 0);
    const nextGuestScore = guestScore + (resolved.winner === "guest" ? 1 : 0);
    const target = Number(r.cardBattleTargetScore ?? CARD_BATTLE_MATCH_TARGET_POINTS);
    if (pickRefs) {
        tx.delete(pickRefs.hostRef);
        tx.delete(pickRefs.guestRef);
    }
    if (resolved.winner !== "draw" &&
        ((resolved.winner === "host" && nextHostScore >= target) ||
            (resolved.winner === "guest" && nextGuestScore >= target))) {
        return {
            status: "completed",
            ...(await applyCardBattleMatchCompletionInTransaction(tx, roomRef, roomId, { ...r, cardBattleNextHostScore: nextHostScore, cardBattleNextGuestScore: nextGuestScore }, resolved.winner, hostCardId, guestCardId, resolved.hostPower, resolved.guestPower)),
        };
    }
    tx.update(roomRef, {
        status: "playing",
        phase: "card_battle_playing",
        cardBattleHostScore: nextHostScore,
        cardBattleGuestScore: nextGuestScore,
        cardBattleRound: Number(r.cardBattleRound ?? 1) + 1,
        cardBattlePickedUids: [],
        cardBattleLastHostCardId: hostCardId,
        cardBattleLastGuestCardId: guestCardId,
        cardBattleLastHostPower: resolved.hostPower,
        cardBattleLastGuestPower: resolved.guestPower,
        cardBattleLastRoundWinner: resolved.winner,
        timeoutEmptyRounds: 0,
        actionDeadlineAt: pvpActionDeadlineTs(Date.now() + CARD_BATTLE_ROUND_REVEAL_MS, cardWindowMs),
        atualizadoEm: firestore_2.FieldValue.serverTimestamp(),
    });
    return {
        status: "round",
        roundWinner: resolved.winner,
        hostScore: nextHostScore,
        guestScore: nextGuestScore,
        hostPower: resolved.hostPower,
        guestPower: resolved.guestPower,
    };
}
...
exports.updateUserAvatar = (0, https_1.onCall)(DEFAULT_CALLABLE_OPTS, async (request) => {
    ...
    if (authPhotoURL) {
        if (!isAllowedUserAvatarUrl(authPhotoURL, uid)) {
            throw new https_1.HttpsError("invalid-argument", "Use uma foto do Google ou do armazenamento oficial do app.");
        }
...
exports.processRewardedAd = (0, https_1.onCall)(DEFAULT_CALLABLE_OPTS, async (request) => {
    ...
    if (!IS_LOCAL_EMULATOR) {
        throw new https_1.HttpsError("failed-precondition", "Recompensas de anúncio em produção precisam da confirmação SSV do AdMob.");
    }
...
exports.processRouletteSpin = (0, https_1.onCall)(DEFAULT_CALLABLE_OPTS, async (request) => {
    ...
    if (mode !== "daily_ad" && mode !== "paid") {
        throw new https_1.HttpsError("invalid-argument", "Modo de giro inválido.");
    }
    const economyConfig = await getEconomy();
    const userRef = db.doc(`${COL.users}/${uid}`);
    const today = dailyKey();
    ...
    const result = await db.runTransaction(async (tx) => {
        ...
        if (mode === "daily_ad") {
            ...
            if (adSnap?.exists) {
                throw new https_1.HttpsError("already-exists", "Este anúncio já foi usado para girar a roleta.");
            }
            const currentDayKey = String(u.rewardedAdsDayKey || "");
            const currentCount = currentDayKey === today ? Math.max(0, Math.floor(Number(u.rewardedAdsCount || 0))) : 0;
            ...
            userPatch.totalAdsAssistidos = firestore_2.FieldValue.increment(1);
            ...
            if (amount > 0) {
                ...
                if (currentBalance < amount) {
                    throw new https_1.HttpsError("failed-precondition", `Saldo insuficiente para girar. Custo: ${amount} ${rewardCurrencyLabel(currency)}.`);
                }
                userPatch[rewardFieldName(currency)] = firestore_2.FieldValue.increment(-amount);
                paidCost = { amount, currency, balanceAfter: currentBalance - amount };
            }
        }
        const economy = (0, gameEconomy_1.resolveMatchEconomy)("roleta", "vitoria", 0, { spinMode: mode, paidCurrency: paidCost?.currency ?? null, paidAmount: paidCost?.amount ?? 0 }, economyConfig.matchRewardOverrides, undefined, economyConfig.rouletteTable);
        const spinMeta = economy.resolvedMetadata;
        const pkStr = typeof spinMeta["roulettePrizeKind"] === "string" ? String(spinMeta["roulettePrizeKind"]).trim() : "";
        const rawCr = spinMeta["chestRarity"];
        const chestRarityResolved = pkStr === "chest" && typeof rawCr === "string" && CHEST_RARITIES.includes(rawCr)
            ? rawCr
            : null;
        const chestRarity = chestRarityResolved;
        const roulettePrizeKind = chestRarityResolved
            ? "chest"
            : pkStr === "gems"
                ? "gems"
                : pkStr === "rewardBalance"
                    ? "rewardBalance"
                    : "coins";
        const grantAmt = roulettePrizeKind === "chest"
            ? 0
            : Math.max(0, Math.floor(Number(spinMeta["rouletteRewardAmount"] ?? spinMeta["serverPrize"] ?? economy.rewardCoins) || 0));
        ...
        if (roulettePrizeKind === "coins" && grantAmt > 0) {
            userPatch.coins = firestore_2.FieldValue.increment(grantAmt);
        }
        if (roulettePrizeKind === "gems" && grantAmt > 0) {
            userPatch.gems = firestore_2.FieldValue.increment(grantAmt);
        }
        if (roulettePrizeKind === "rewardBalance" && grantAmt > 0) {
            userPatch.rewardBalance = firestore_2.FieldValue.increment(grantAmt);
        }
        userPatch.totalPartidas = firestore_2.FieldValue.increment(1);
        userPatch.totalVitorias = firestore_2.FieldValue.increment(1);
        userPatch.xp = firestore_2.FieldValue.increment(15);
        tx.update(userRef, userPatch);
        tx.set(matchRef, {
            id: matchRef.id,
            gameId: "roleta",
            ...
            startedAt: null,
            finishedAt: firestore_2.Timestamp.now(),
    ...
    return {
        ...
        grantedChest,
        chestNotGranted: result.roulettePrizeKind === "chest" && Boolean(result.chestRarity) && grantedChest === null,
    };
});
exports.prepareRewardedAdSession = (0, https_1.onCall)(DEFAULT_CALLABLE_OPTS, async (request) => {
    ...
    let chestIdForSession = null;
    if (placementId === RAFFLE_NUMBER_PLACEMENT_ID) {
        const raffleIdForAd = String(request.data?.raffleId || "").trim();
        if (!raffleIdForAd) {
            throw new https_1.HttpsError("invalid-argument", "Informe raffleId para preparar o anúncio do sorteio.");
        }
        ...
        const raffleForAd = raffleDocFromFirestore(raffleSnapForAd.id, (raffleSnapForAd.data() || {}));
        if (raffleForAd.status !== "active" || raffleForAd.entryMode !== "rewarded_ad") {
            throw new https_1.HttpsError("failed-precondition", "Este sorteio não está aberto para números por anúncio.");
        }
    ...
    if (placementId === CHEST_SPEEDUP_PLACEMENT_ID) {
        ...
        if (!chestIdForAd || chestIdForAd.includes("/")) {
            throw new https_1.HttpsError("invalid-argument", "Informe o baú para preparar o anúncio.");
        }
...
exports.finalizeMatch = (0, https_1.onCall)(DEFAULT_CALLABLE_OPTS, async (request) => {
    ...
    const effectiveResult = gameId === "bau" ? "vitoria" : resultado;
    const economyConfig = await getEconomy();
    ...
    const appliedResult = await db.runTransaction(async (tx) => {
        ...
        tx.update(userRef, {
            totalSoloPartidas: firestore_2.FieldValue.increment(1),
            atualizadoEm: firestore_2.FieldValue.serverTimestamp(),
...
exports.startChestUnlock = (0, https_1.onCall)(DEFAULT_CALLABLE_OPTS, async (request) => {
    ...
    return db.runTransaction(async (tx) => {
        ...
        if (currentStatus === "locked" &&
            ...
            const mySlot = chest.slotIndex;
            const hasLockedLowerSlot = normalizedItems.some((other) => other.id !== chestId &&
                other.status === "locked" &&
            ...
            if (hasLockedLowerSlot) {
                ...
                throw new https_1.HttpsError("failed-precondition", "Abra primeiro o baú no slot de menor número (comece pelo slot 1).");
            }
        }
...
exports.speedUpChestUnlock = (0, https_1.onCall)(DEFAULT_CALLABLE_OPTS, async (request) => {
    ...
    if (!IS_LOCAL_EMULATOR && !adSessionId) {
        throw new https_1.HttpsError("failed-precondition", "Validação do anúncio ainda não foi concluída pelo AdMob.");
    }
    ...
    return db.runTransaction(async (tx) => {
        ...
        if (adSessionRef) {
            if (!adSessionSnap?.exists)
                throw new https_1.HttpsError("failed-precondition", "Sessão de anúncio inexistente.");
            ...
            if (String(session.userId || "") !== uid || String(session.placementId || "") !== CHEST_SPEEDUP_PLACEMENT_ID || String(session.chestId || "") !== chestId) {
                throw new https_1.HttpsError("permission-denied", "Sessão de anúncio inválida para este baú.");
            }
            if (String(session.status || "") !== "recompensado") {
                throw new https_1.HttpsError("failed-precondition", "A recompensa do anúncio ainda está pendente.");
            }
        }
        const userData = userSnap.data();
        if (userData.banido) {
        ...
        if (adSessionRef)
            tx.update(adSessionRef, { status: "consumido", consumedAt: firestore_2.FieldValue.serverTimestamp(), updatedAt: firestore_2.FieldValue.serverTimestamp() });
...
exports.requestRewardClaim = (0, https_1.onCall)(DEFAULT_CALLABLE_OPTS, async (request) => {
    ...
    if (request.auth?.token?.email_verified !== true) {
        throw new https_1.HttpsError("failed-precondition", "Verifique seu e-mail antes de solicitar um saque.");
    }
    let parsed;
    try {
    ...
    await db.runTransaction(async (tx) => {
        ...
        if (u.banido === true || u.riscoFraude === "alto" || u.riscoFraude === "medio") {
            throw new https_1.HttpsError("permission-denied", "Saque temporariamente retido para análise de segurança.");
        }
        if (typeof u.activeRewardClaimId === "string" && u.activeRewardClaimId.trim()) {
            throw new https_1.HttpsError("failed-precondition", "Você já possui um saque em análise.");
        }
...
exports.reviewRewardClaim = (0, https_1.onCall)(DEFAULT_CALLABLE_OPTS, async (request) => {
    ...
    await db.runTransaction(async (tx) => {
        ...
        if (!userId || !Number.isFinite(value) || value <= 0) {
            throw new https_1.HttpsError("failed-precondition", "Pedido com dados financeiros inválidos.");
        }
        const userRef = db.doc(`${COL.users}/${userId}`);
        const userSnap = await tx.get(userRef);
        if (!userSnap.exists) {
            throw new https_1.HttpsError("failed-precondition", "Usuário do pedido não encontrado.");
        }
        ...
        if (status === "aprovado" && !retained) {
            ...
            tx.update(userRef, {
                rewardBalance: firestore_2.FieldValue.increment(-value),
                atualizadoEm: firestore_2.FieldValue.serverTimestamp(),
            });
            addWalletTxInTx(tx, {
        ...
        if (status === "recusado" && retained) {
            ...
            tx.update(userRef, {
                rewardBalance: firestore_2.FieldValue.increment(value),
                activeRewardClaimId: firestore_2.FieldValue.delete(),
                atualizadoEm: firestore_2.FieldValue.serverTimestamp(),
            });
            addWalletTxInTx(tx, {
...
function isAllowedComprovanteUrl(raw, claimId) {
    const u = raw.trim();
...
exports.confirmRewardClaimPix = (0, https_1.onCall)(DEFAULT_CALLABLE_OPTS, async (request) => {
    ...
    await db.runTransaction(async (tx) => {
        ...
        if (!claimSnap.exists || claimSnap.data()?.status !== "aprovado") {
            throw new https_1.HttpsError("failed-precondition", "O pedido já foi atualizado.");
        }
        tx.update(ref, {
            status: "confirmado",
            comprovanteUrl,
        ...
        if (userId)
            tx.update(db.doc(`${COL.users}/${userId}`), { activeRewardClaimId: firestore_2.FieldValue.delete() });
    });
    return { ok: true };
});
...
async function closeRaffleIfDue(raffleId, nowMs) {
    ...
    return db.runTransaction(async (tx) => {
        ...
        if (!shouldAutoCloseRaffle(raffle, nowMs))
            return false;
        tx.set(ref, buildCloseRafflePayload(raffle, firestore_2.Timestamp.fromMillis(nowMs)), { merge: true });
...
async function drawClosedRaffle(raffleId, winningNumberRaw) {
    ...
    if (scheduledResultAt && scheduledResultAt.toMillis() > Date.now()) {
        throw new https_1.HttpsError("failed-precondition", `O número oficial só pode ser lançado após ${scheduledResultAt
            .toDate()
    ...
    const drawResult = await db.runTransaction(async (tx) => {
        ...
        const winnerData = winnerUserSnap.exists
            ? (winnerUserSnap.data() || {})
            : {};
        const winnerName = typeof winnerData.nome === "string" && winnerData.nome.trim() ? winnerData.nome.trim() : null;
        const winnerUsername = typeof winnerData.username === "string" && winnerData.username.trim()
    ...
    const payout = await db.runTransaction(async (tx) => {
        ...
        tx.set(raffleRef, {
            status: "paid",
            winnerName: typeof u.nome === "string" && u.nome.trim() ? u.nome.trim() : drawResult.winnerName ?? null,
            winnerUsername: typeof u.username === "string" && u.username.trim()
                ? u.username.trim()
...
exports.listPublishedRaffles = (0, https_1.onCall)(DEFAULT_CALLABLE_OPTS, async (request) => {
    const uid = request.auth?.uid;
    assertAuthed(uid);
...
exports.purchaseRaffleNumbers = (0, https_1.onCall)(DEFAULT_CALLABLE_OPTS, async (request) => {
    ...
    const previewRaffle = raffleDocFromFirestore(previewSnap.id, (previewSnap.data() || {}));
    ...
    if (previewEntryMode === "ticket") {
        if (rewardedAdSessionIdIn || adCompletionToken) {
            throw new https_1.HttpsError("invalid-argument", "Este sorteio aceita apenas pagamento em TICKET. Não envie dados de anúncio.");
        }
    }
    else {
        if (quantity !== 1) {
            throw new https_1.HttpsError("invalid-argument", "Neste sorteio cada anúncio válido libera exatamente 1 número por vez.");
        }
    ...
    const result = await db.runTransaction(async (tx) => {
        ...
        if (entryMode === "ticket" && (rewardedAdSessionIdIn || adCompletionToken)) {
            throw new https_1.HttpsError("failed-precondition", "Este sorteio não está em modo anúncio.");
        }
        if (entryMode === "rewarded_ad" && quantity !== 1) {
            throw new https_1.HttpsError("invalid-argument", "Quantidade inválida para sorteio por anúncio.");
        }
        const nowMs = Date.now();
        const nowTs = firestore_2.Timestamp.fromMillis(nowMs);
        ...
        if (entryMode === "rewarded_ad") {
            ...
            if (rewardedAdSessionIdIn) {
                ...
                if (!sessSnap.exists) {
                    throw new https_1.HttpsError("failed-precondition", "Sessão de anúncio não encontrada.");
                }
                const sd = (sessSnap.data() || {});
                if (String(sd.userId || "") !== uid) {
                    throw new https_1.HttpsError("permission-denied", "Sessão de anúncio não pertence a você.");
                }
                ...
                if (sd.raffleConsumedPurchaseId) {
                    throw new https_1.HttpsError("failed-precondition", "Este anúncio já foi usado para obter um número.");
                }
                tx.set(sessionRef, {
                    raffleConsumedAt: firestore_2.FieldValue.serverTimestamp(),
                ...
                if (!adSnap.exists) {
                    throw new https_1.HttpsError("failed-precondition", "Comprovante de anúncio não encontrado. Assista ao anúncio e tente novamente.");
                }
                const ad = (adSnap.data() || {});
                if (String(ad.userId || "") !== uid) {
                    throw new https_1.HttpsError("permission-denied", "Comprovante de anúncio inválido.");
                }
                if (String(ad.rewardKind || "") !== "raffle_number") {
                    throw new https_1.HttpsError("failed-precondition", "Este comprovante não é de sorteio por anúncio.");
                }
                ...
                throw new https_1.HttpsError("failed-precondition", "Comprovante de anúncio obrigatório.");
            }
        }
        const winnerName = typeof u.nome === "string" && u.nome.trim() ? String(u.nome).trim() : null;
        const winnerUsername = typeof u.username === "string" && u.username.trim() ? String(u.username).trim() : null;
        if (entryMode === "ticket") {
            addWalletTxInTx(tx, {
        ...
        if (entryMode === "ticket") {
            raffleUpdate.soldTicketsRevenue = firestore_2.FieldValue.increment(ticketCost);
        }
        if (raffle.allocationMode === "random") {
            ...
            while (picked.length < quantity && attempts < maxAttempts) {
                attempts += 1;
                const cand = (0, node_crypto_1.randomInt)(0, raffle.releasedCount);
            ...
            if (picked.length < quantity) {
                const scanStart = (0, node_crypto_1.randomInt)(0, raffle.releasedCount);
            ...
            if (picked.length < quantity) {
                throw new https_1.HttpsError("failed-precondition", "Não há números disponíveis suficientes para esta compra.");
            }
        ...
        const { hits: instantPrizeHits, tiers: updatedInstantPrizeTiers, } = resolveInstantPrizeHitsForPurchase({
            raffle,
            purchaseId,
            userId: uid,
            winnerName,
        ...
        for (const currency of ["coins", "gems", "rewardBalance"]) {
            if (userDeltas[currency] !== 0) {
                userUpdate[rewardFieldName(currency)] = firestore_2.FieldValue.increment(userDeltas[currency]);
            }
        }
        if (entryMode === "rewarded_ad") {
            ...
            userUpdate.raffleAdLastClaimAt = {
                [raffleId]: firestore_2.FieldValue.serverTimestamp(),
            };
        }
        tx.set(userRef, userUpdate, { merge: true });
        tx.set(raffleRef, raffleUpdate, { merge: true });
        if (raffleUpdate.status === "closed" &&
...
exports.adminCreateOrUpdateRaffle = (0, https_1.onCall)(DEFAULT_CALLABLE_OPTS, async (request) => {
    ...
    const entryMode = entryModeInput === "rewarded_ad" ? "rewarded_ad" : "ticket";
    if (entryMode === "rewarded_ad") {
        maxPerPurchase = 1;
    }
    const rewardedAdCooldownSeconds = normalizeRaffleRewardedAdCooldownSeconds(request.data?.rewardedAdCooldownSeconds, entryMode);
    ...
    const rawEndsAt = endsAtMs != null && Number.isFinite(endsAtMs) ? firestore_2.Timestamp.fromMillis(endsAtMs) : null;
    ...
    if (status === "active" && !startsAt) {
        throw new https_1.HttpsError("invalid-argument", "Defina a data de início antes de ativar o sorteio.");
    }
    if (status === "active" && requestedScheduleMode === "date_range" && !endsAt) {
        throw new https_1.HttpsError("invalid-argument", "Defina a data final para o modo com início e fim.");
    }
    ...
    if (totalInstantPrizeQuantity > RAFFLE_MAX_INSTANT_PRIZE_HITS) {
        throw new https_1.HttpsError("invalid-argument", `O total de números premiados não pode ultrapassar ${RAFFLE_MAX_INSTANT_PRIZE_HITS}.`);
    }
    if (totalInstantPrizeQuantity > releasedCount) {
        throw new https_1.HttpsError("invalid-argument", "A quantidade total de números premiados não pode passar da faixa liberada.");
    }
    const system = await getRaffleSystemConfig();
    ...
    await db.runTransaction(async (tx) => {
        ...
        const resolvedAllocation = allocationInput === "sequential"
            ...
            : allocationInput === "random"
                ...
                : prev
                    ? prev.allocationMode
                    : "random";
        const resolvedScheduleMode = scheduleModeInput === "until_sold_out"
            ? "until_sold_out"
        ...
        if (prev) {
            ...
            if (prev.soldCount > 0 && entryMode !== prev.entryMode) {
                throw new https_1.HttpsError("failed-precondition", "Não é possível alterar o modo de inscrição (TICKET / anúncio) após existirem números distribuídos.");
            }
            if (prev.soldCount > 0 &&
                instantPrizeConfigSignature(prev.instantPrizeTiers) !== instantPrizeConfigSignature(requestedInstantPrizeTiers)) {
                throw new https_1.HttpsError("failed-precondition", "Não é possível alterar os números premiados após existirem vendas.");
            }
        }
        if (status === "active") {
        ...
        const payload = {
            ...
            instantPrizeTiers: prev && prev.soldCount > 0
...
exports.adminCloseRaffle = (0, https_1.onCall)(DEFAULT_CALLABLE_OPTS, async (request) => {
    ...
    const ref = db.doc(`${COL.raffles}/${raffleId}`);
    const activeLockRef = raffleActiveLockRef();
    const closedAt = firestore_2.Timestamp.now();
    await db.runTransaction(async (tx) => {
        const [snap, activeLockSnap] = await Promise.all([tx.get(ref), tx.get(activeLockRef)]);
        if (!snap.exists)
            throw new https_1.HttpsError("not-found", "Sorteio não encontrado.");
...
exports.adminDrawRaffle = (0, https_1.onCall)(DEFAULT_CALLABLE_OPTS, async (request) => {
    ...
    await db.runTransaction(async (tx) => {
        ...
        const raffle = raffleDocFromFirestore(snap.id, (snap.data() || {}));
        if (raffle.status !== "closed") {
            throw new https_1.HttpsError("failed-precondition", "Somente sorteios encerrados podem receber o número oficial.");
        }
    });
...
function matchmakingQueueShard(uid) {
    let hash = 2166136261;
...
exports.joinAutoMatch = (0, https_1.onCall)(MULTIPLAYER_CALLABLE_OPTS, async (request) => {
    ...
    const cardBattleMatchWinMs = pvpChoiceWindowMs(econMatch.pvpChoiceSeconds, "card_battle");
    try {
        const result = await db.runTransaction(async (tx) => {
            ...
            const matchmakingSlotStillActive = (snap) => {
                if (!snap.exists)
                    return false;
                const sd = snap.data();
                if (String(sd.queueStatus) !== "waiting")
                    return false;
            ...
            const initialActionDeadlineAt = gameId === "reaction_tap" && reactionGoLiveAt
                ...
                : pvpActionDeadlineTs(Date.now(), gameId === "ppt"
                    ? pptMatchWinMs
                    : gameId === "quiz"
                        ? quizMatchWinMs
                        : gameId === "card_battle"
                            ? cardBattleMatchWinMs
                            : reactionMatchWinMs);
            tx.set(roomRef, {
                id: roomRef.id,
                gameId,
                ...
                ...(gameId === "card_battle"
                    ? {
                        status: "playing",
                        phase: "card_battle_playing",
                        cardBattleHostScore: 0,
                        cardBattleGuestScore: 0,
                        cardBattleTargetScore: CARD_BATTLE_MATCH_TARGET_POINTS,
                        cardBattleRound: 1,
                        cardBattlePickedUids: [],
                    }
                    : {}),
...
exports.submitCardBattleCard = (0, https_1.onCall)(MULTIPLAYER_CALLABLE_OPTS, async (request) => {
    const uid = request.auth?.uid;
    assertAuthed(uid);
    const roomId = String(request.data?.roomId || "").trim();
    const cardIdRaw = String(request.data?.cardId || "").trim().toLowerCase();
    if (!roomId || !isCardBattleCardId(cardIdRaw)) {
        throw new https_1.HttpsError("invalid-argument", "roomId ou carta invalidos.");
    }
    const cardId = cardIdRaw;
    const roomRef = db.doc(`${COL.gameRooms}/${roomId}`);
    const choicesColl = roomRef.collection("card_battle_choices");
    const econCardSubmit = await getEconomy();
    const cardSubmitWindowMs = pvpChoiceWindowMs(econCardSubmit.pvpChoiceSeconds, "card_battle");
    const result = await db.runTransaction(async (tx) => {
        ...
        const room = roomSnap.data();
        if (String(room.gameId) !== "card_battle") {
            throw new https_1.HttpsError("failed-precondition", "Esta sala nao e Batalha de cartas.");
        }
        if (uid !== room.hostUid && uid !== room.guestUid) {
            throw new https_1.HttpsError("permission-denied", "Voce nao esta nesta sala.");
        }
        if (room.cardBattleRewardsApplied === true || room.phase === "completed" || room.status === "completed") {
            throw new https_1.HttpsError("failed-precondition", "Partida ja finalizada.");
        }
        ...
        const myChoiceRef = choicesColl.doc(uid);
        const otherChoiceRef = choicesColl.doc(otherUid);
        const [myChoiceSnap, otherChoiceSnap] = await Promise.all([
            tx.get(myChoiceRef),
            tx.get(otherChoiceRef),
        ]);
        if (myChoiceSnap.exists) {
            throw new https_1.HttpsError("failed-precondition", "Voce ja escolheu carta nesta rodada.");
        }
        const picked = new Set(Array.isArray(room.cardBattlePickedUids)
            ? room.cardBattlePickedUids.map((x) => String(x))
            : []);
        picked.add(uid);
        if (!otherChoiceSnap.exists) {
            tx.set(myChoiceRef, {
                uid,
                cardId,
                createdAt: firestore_2.FieldValue.serverTimestamp(),
            ...
            tx.update(roomRef, {
                status: "playing",
                phase: "card_battle_waiting",
                cardBattlePickedUids: Array.from(picked),
                timeoutEmptyRounds: 0,
                atualizadoEm: firestore_2.FieldValue.serverTimestamp(),
            });
            return { status: "queued" };
        }
        const otherCardId = String(otherChoiceSnap.data()?.cardId || "timeout");
        const hostCardId = uid === hostUid ? cardId : otherCardId;
        const guestCardId = uid === guestUid ? cardId : otherCardId;
        return applyCardBattleRoundResultInTransaction(tx, roomRef, roomId, room, hostCardId, guestCardId, cardSubmitWindowMs, { hostRef: choicesColl.doc(hostUid), guestRef: choicesColl.doc(guestUid) });
    });
    if (result.status === "completed") {
        await postCardBattleRankingFromWinner(roomId, result.hostUid, result.guestUid, result.matchWinner, result.hostPower, result.guestPower);
    }
    return result;
});
...
exports.forfeitPvpRoom = (0, https_1.onCall)(MULTIPLAYER_CALLABLE_OPTS, async (request) => {
    ...
    const result = await db.runTransaction(async (tx) => {
        ...
        if (gameId !== "ppt" && gameId !== "quiz" && gameId !== "reaction_tap" && gameId !== "card_battle") {
        ...
        if (gameId === "card_battle") {
            const hostUid = String(r.hostUid);
            const matchWinner = uid === hostUid ? "guest" : "host";
            const hostPower = matchWinner === "host" ? 24 : 0;
            const guestPower = matchWinner === "guest" ? 24 : 0;
            const out = await applyCardBattleMatchCompletionInTransaction(tx, roomRef, roomId, {
                ...r,
                cardBattleNextHostScore: Number(r.cardBattleHostScore ?? 0) + (matchWinner === "host" ? 1 : 0),
                cardBattleNextGuestScore: Number(r.cardBattleGuestScore ?? 0) + (matchWinner === "guest" ? 1 : 0),
            }, matchWinner, matchWinner === "host" ? "guardiao" : "timeout", matchWinner === "guest" ? "guardiao" : "timeout", hostPower, guestPower, uid);
            return { applied: true, gameId: "card_battle", ...out };
        }
    ...
    if (result.applied) {
        if (result.gameId === "ppt") {
            ...
            if (result.gameId === "card_battle") {
                await postCardBattleRankingFromWinner(roomId, result.hostUid, result.guestUid, result.matchWinner, result.hostPower, result.guestPower, { forfeitedByUid: uid });
                return {
                    ok: true,
                    applied: result.applied,
...
async function resolveExpiredPvpRoom(roomRef, roomId, actorUid) {
    ...
    const cardBattleTimeoutMs = pvpChoiceWindowMs(econTimeout.pvpChoiceSeconds, "card_battle");
    const result = await db.runTransaction(async (tx) => {
        const rs = await tx.get(roomRef);
        if (!rs.exists) {
            return { kind: "noop" };
        ...
        if (gameId === "card_battle") {
            ...
            const hostUid = String(r.hostUid);
            const guestUid = String(r.guestUid);
            const [hostChoiceSnap, guestChoiceSnap] = await Promise.all([
                tx.get(choicesColl.doc(hostUid)),
                tx.get(choicesColl.doc(guestUid)),
            ]);
            if (hostChoiceSnap.exists && guestChoiceSnap.exists) {
                return { kind: "noop" };
            }
            if (!hostChoiceSnap.exists && !guestChoiceSnap.exists) {
                const strikes = Math.max(0, Number(r.timeoutEmptyRounds ?? 0));
                if (strikes >= 1) {
                    await applyGenericPvpTimeoutVoidInTransaction(tx, roomRef, r, {
                        cardBattleOutcome: "draw",
                        cardBattleLastRoundWinner: "draw",
                        cardBattlePickedUids: [],
                        cardBattleRewardsApplied: true,
                        cardBattleMatchWinner: firestore_2.FieldValue.delete(),
                        timeoutEmptyRounds: 0,
                    });
                    return { kind: "void", gameId };
                }
                tx.update(roomRef, {
                    ...
                    cardBattleLastRoundWinner: "draw",
                    timeoutEmptyRounds: strikes + 1,
                    actionDeadlineAt: pvpActionDeadlineTs(Date.now(), cardBattleTimeoutMs),
                    atualizadoEm: firestore_2.FieldValue.serverTimestamp(),
                });
                return { kind: "card_battle_round" };
            }
            const hostCard = hostChoiceSnap.exists
                ? String(hostChoiceSnap.data()?.cardId || "timeout")
                : "timeout";
            const guestCard = guestChoiceSnap.exists
                ? String(guestChoiceSnap.data()?.cardId || "timeout")
                : "timeout";
            const out = await applyCardBattleRoundResultInTransaction(tx, roomRef, roomId, r, hostCard, guestCard, cardBattleTimeoutMs, { hostRef: choicesColl.doc(hostUid), guestRef: choicesColl.doc(guestUid) });
            return out.status === "completed"
                ? { kind: "card_battle_match", ...out }
                : { kind: "card_battle_round" };
        }
        if (gameId === "reaction_tap") {
            const resultsColl = roomRef.collection("reaction_results");
            const hostUid = String(r.hostUid);
    ...
    else if (result.kind === "card_battle_match") {
        await postCardBattleRankingFromWinner(roomId, result.hostUid, result.guestUid, result.matchWinner, result.hostPower, result.guestPower);
    }
    return result;
}
...
exports.riskAnalysisOnUserEvent = (0, https_1.onCall)(DEFAULT_CALLABLE_OPTS, async (request) => {
    ...
    if (!/^[a-zA-Z0-9:_-]+$/.test(tipo)) {
        throw new https_1.HttpsError("invalid-argument", "Tipo de evento invÃ¡lido.");
    }
    const rawDetails = request.data?.detalhes;
    const detalhes = rawDetails && typeof rawDetails === "object" && !Array.isArray(rawDetails)
        ? rawDetails
        : {};
    let detailsSize = 0;
    try {
        ...
        throw new https_1.HttpsError("invalid-argument", "Detalhes do evento invÃ¡lidos.");
    }
    if (detailsSize > 2000) {
...
exports.reapStaleAutoMatchSlots = (0, scheduler_1.onSchedule)({ ...DEFAULT_SCHEDULE_OPTS, schedule: "* * * * *" }, async () => {
    const cutoff = firestore_2.Timestamp.fromMillis(Date.now() - MATCHMAKING_SLOT_STALE_MS);
    const snap = await db
        .collection(COL.multiplayerSlots)
        .where("queueStatus", "==", "waiting")
...
exports.getArenaOverallRanking = (0, https_1.onCall)(DEFAULT_CALLABLE_OPTS, async (request) => {
    ...
    const byGameRows = {
        ...
        card_battle: buildArenaOverallRows(statsByUser, usersById, "card_battle"),
    };
    const packRows = (rows) => {
    ...
    return {
        ...
        byGame: {
            ...
            card_battle: packRows(byGameRows.card_battle),
        },
    };
});
...
async function pruneClanMessages(clanId, keep = CLAN_MESSAGE_RETENTION_LIMIT) {
...
function clanScopedJoinRequestRef(clanId, uid) {
...
function extractClanAssetPathFromUrl(rawUrl, ownerUid) {
...
async function deleteClanAssetIfExists(rawUrl, ownerUid) {
...
function createArenaOverallAccumulator() {
    return {
        ...
        byGame: {
            ...
            card_battle: createArenaOverallStats(),
        },
    };
}
...
exports.createClan = (0, https_1.onCall)(DEFAULT_CALLABLE_OPTS, async (request) => {
    ...
    if (description)
        assertAllowedPublicName(description, "Descrição do clã");
...
exports.leaveClan = (0, https_1.onCall)(DEFAULT_CALLABLE_OPTS, async (request) => {
    ...
    if (role === "owner") {
        ...
        if (ownerUid !== uid) {
            throw new https_1.HttpsError("permission-denied", "Dados do clã inconsistentes. Contate o suporte.");
        }
        ...
        if (rosterSnap.size === 1 && rosterSnap.docs[0].id !== uid) {
            throw new https_1.HttpsError("permission-denied", "Dados do membro do clã inconsistentes.");
        }
        ...
        await db.runTransaction(async (tx) => {
            ...
            if (String(mData.clanId || "") !== clanId || String(mData.role || "") !== "owner") {
                throw new https_1.HttpsError("permission-denied", "Somente o fundador pode encerrar o clã.");
            }
            if (String(cData.ownerUid || "").trim() !== uid) {
                throw new https_1.HttpsError("permission-denied", "Operação não permitida.");
            }
            tx.delete(clanMembershipRef(uid));
        });
...
exports.updateClanSettings = (0, https_1.onCall)(DEFAULT_CALLABLE_OPTS, async (request) => {
    ...
    if (nextName !== undefined)
        assertAllowedPublicName(nextName, "Nome do clã");
    if (nextTag !== undefined)
        assertAllowedPublicName(nextTag, "TAG do clã");
    if (description)
        assertAllowedPublicName(description, "Descrição do clã");
    ...
    if (rawAvatarUrl === null) {
        ...
        if (!isOwnClanAssetUrl(avatarUrl, uid))
            throw new https_1.HttpsError("permission-denied", "Avatar deve ser um arquivo do seu clã.");
    ...
    if (rawCoverUrl === null) {
        ...
        if (!isOwnClanAssetUrl(coverUrl, uid))
            throw new https_1.HttpsError("permission-denied", "Capa deve ser um arquivo do seu clã.");
    ...
    for (const [collection, field] of [
        ...
        if (!snap.empty) {
            ...
            snap.docs.forEach((doc) => batch.update(doc.ref, {
                ...
                chavePix: firestore_2.FieldValue.delete(),
                comprovanteUrl: firestore_2.FieldValue.delete(),
